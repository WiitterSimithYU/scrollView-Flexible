...
@implementation KJDisconnectView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.titleLb = [self creatLableWithTitle:@"网络异常" font:15 color:[UIColor darkGrayColor]];
        [self addSubview:self.titleLb];
        
        self.detailLb = [self creatLableWithTitle:@"哎呦，被挤爆了，请检查网络重试" font:15 color:[UIColor darkGrayColor]];
...
- (void)refreshBtnClick:(UIButton *)button {
    ...
    if (self.retryConnectBlock) {
        self.retryConnectBlock();
    }
}
